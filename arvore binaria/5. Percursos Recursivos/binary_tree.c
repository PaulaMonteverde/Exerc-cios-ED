...
//consertar as funções recursivas, tirar as funções;
//binary_tree_destroy, binary_tree_print, intervalo, recursivas
...
KeyValPair *key_val_pair_copy(KeyValPair *original) {
    if (original == NULL) {
        return NULL;
    }
    KeyValPair *copy = (KeyValPair *)calloc(1, sizeof(KeyValPair));
...
Vector *binary_tree_levelorder_traversal(BinaryTree *bt) {
    ...
    while (vector_size(queue) > 0) {
        Node *current = (Node *)vector_get(queue, 0);
        vector_remove(queue, 0); 

        KeyValPair *copy = key_val_pair_copy(current->pair);
...
void inorder_recursive(Node *node, Vector *v){
    ...
    KeyValPair*copy = key_val_pair_copy(node->pair);
